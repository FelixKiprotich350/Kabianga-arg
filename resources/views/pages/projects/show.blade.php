            ...
            <div class="col-lg-8 mb-4">
                ...
                <div class="form-card mb-4">
                    <div class="d-flex justify-content-between align-items-center mb-3">
                        ...
                        <span
                            class="badge {{ $project->projectstatus == \App\Models\ResearchProject::STATUS_ACTIVE ? 'bg-success' : ($project->projectstatus == \App\Models\ResearchProject::STATUS_COMPLETED ? 'bg-info' : 'bg-warning') }}">
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <p class="fw-medium">
                                {{ $project->proposal->commencingdate ? date('M d, Y', strtotime($project->proposal->commencingdate)) : 'N/A' }}
                            </p>
                        </div>
                        <div class="col-md-6 mb-3">
                            <label class="form-label text-muted">End Date</label>
                            <p class="fw-medium">
                                {{ $project->proposal->terminationdate ? date('M d, Y', strtotime($project->proposal->terminationdate)) : 'N/A' }}
                            </p>
                        </div>
                ...
                <form id="progressForm" class="ajax-form">
                    ...
                    <div class="modal-body">
                        <div class="mb-3">
                            ...
                            <textarea class="form-control" name="report" rows="6" placeholder="Describe your research progress..."
                                required></textarea>
                        </div>
                    </div>
        ...
        $(document).ready(function() {
            ...
            window.pauseProject = function() {
                if (confirm('Pause this project?')) {
                    $.ajax({
                        url: "{{ route('api.projects.pauseproject', $project->researchid) }}",
                        type: 'PATCH',
                        data: {
            ...
            window.resumeProject = function() {
                if (confirm('Resume this project?')) {
                    $.ajax({
                        url: "{{ route('api.projects.resumeproject', $project->researchid) }}",
                        type: 'PATCH',
                        data: {
                            _token: $('meta[name="csrf-token"]').attr('content')
                        }
            ...
            window.completeProject = function() {
                if (confirm('Mark this project as complete?')) {
                    $.ajax({
                        url: "{{ route('api.projects.completeproject', $project->researchid) }}",
                        type: 'PATCH',
                        data: {
                            _token: $('meta[name="csrf-token"]').attr('content')
                        }
            ...
            window.cancelProject = function() {
                if (confirm('Cancel this project? This action cannot be undone.')) {
                    $.ajax({
                        url: "{{ route('api.projects.cancelproject', $project->researchid) }}",
                        type: 'PATCH',
                        data: {
                            _token: $('meta[name="csrf-token"]').attr('content')
                        }
